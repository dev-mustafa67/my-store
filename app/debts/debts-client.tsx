...
import { BookText, Phone, CheckCircle2, MessageSquare, Clock } from 'lucide-react';
      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <form onSubmit={addDebt} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-3">
          ...
          <div className="flex gap-3 flex-wrap">
            <input 
              ...
              className="h-11 px-3 rounded-lg border border-gray-200 flex-1 min-w-[140px] text-sm outline-none" 
            />
            <input 
            ...
            <input 
              ...
              className="h-11 px-3 rounded-lg border border-gray-200 flex-1 min-w-[100px] text-sm outline-none" 
            />
          </div>
          <button className="w-full sm:w-auto px-6 h-11 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 text-sm">
            تسجيل الدين
          </button>
        </form>

        {/* التبديل بين الديون المستحقة والمسددة */}
        <div className="flex gap-2 border-b border-gray-200 pb-2">
          <button
            onClick={() => setTab('unpaid')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition ${
              tab === 'unpaid' ? 'bg-indigo-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
          ...
          <button
            onClick={() => setTab('paid')}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition ${
              tab === 'paid' ? 'bg-green-600 text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'
            }`}
          >
        ...
        <div className="space-y-3">
          {displayedList.map((d) => (
            <div 
              key={d.id} 
              className={`flex flex-col sm:flex-row justify-between sm:items-center p-4 rounded-2xl text-sm shadow-sm border gap-3 ${
                d.paid ? 'bg-green-50/60 border-green-100' : 'bg-white border-gray-100'
              }`}
            >
              <div>
                ...
                <p className="text-gray-500 text-xs mt-1 flex items-center gap-3 flex-wrap">
                  {d.customer_phone && (
                    <span className="flex items-center gap-1"><Phone size={11} /> {d.customer_phone}</span>
                  )}
                  <span>{d.note}</span>
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                ...
                {!d.paid ? (
                  <button 
                    ...
                    className="px-4 h-8 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700 transition"
                  >
                    إغلاق الدين
                  </button>
                ) : (
                  <span className="text-xs text-gray-400 font-medium">
                    سُدد في: {d.paid_at ? new Date(d.paid_at).toLocaleDateString('ar-IQ') : '—'}
                  </span>
                )}
              </div>
            </div>
          ...
          {displayedList.length === 0 && (
            <p className="text-gray-400 text-sm text-center py-6">لا توجد ديون في هذه القائمة.</p>
          )}
        </div>
      </div>
    </div>
